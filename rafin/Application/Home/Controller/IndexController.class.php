...
//  		$jlrr = $jl -> add($wherelg);
	...
	public function jrpointdwdetailfws(){
		...
		$jpsr = $js -> field('persname,gonghao,dwname,zhiwu,jgh,pointcontentid,sum('.$point_sum.') as sums') -> where($where) -> group('persname,gonghao,dwname,zhiwu,jgh,pointcontentid') -> select();
...
//	    	}
	...
	public function persjghmodify(){
		$gh = $_GET['gonghao'];
		cookie('gonghao',$gh);

		$pc = M('jrpointcontent');
		$pcrr = $pc -> select();
		$this->assign('pcrr',$pcrr);

		$this->display();
	}
    public function persjghmodifys(){
		$where['gonghao'] = cookie('gonghao');
		$limit = $_GET['limit'];
    	$page = $_GET['page'];
    	$first = $limit * ($page-1);
    	$date_date = $_GET['date'];
    	$pc = $_GET['pointcontentid'];
    	if($date_date == null){
		...
		$data_where = json_encode($where);
    	$json = '{"code":0,"msg":"","count":'.$count.',"data":'.$data.'}';
    	
		echo $json;
		// echo $data_where;
	}
    public function persjghmodifysuc(){
		// var_dump($_POST);
		$pointsumid = $_POST['pointsumid'];

		$where['point'] = $_POST['point'];
		$where['sum'] = $_POST['point'] * $_POST['score'];

		$ps = M('jrpointsum');
		$psrr = $ps -> where("pointsumid = '$pointsumid'") -> save($where);

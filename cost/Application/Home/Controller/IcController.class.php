...
class IcController extends Controller {
	...
	public function equipment_tablemat(){
		// var_dump($_GET);
		$where['equipmentid'] = $_GET['equipmentid'];
		$ieq = M('icequipment');
		$gather = $ieq -> where($where) -> select();
		$data['tablemat_classid'] = $gather[0]['tablemat_classid'];
	...
	public function welcomes(){
		...
		$where['stats'] = 0;
		$ieqsr = $ieqs -> where($where) -> select();
		$ieqsr = $this->deviceapi($ieqsr);
		// var_dump($ieqsr);
		$ieq = M('icequipment');
		// $ieqr = $ieq -> where($where) -> select();
		...
		foreach($drr as &$value){
			...
			foreach($ieqsr as &$val){
				...
				if($value['dwnames'] == $val['jgh']){
					...
					// $value['gather'] = $ieqr[0]['gather'];
				}
			}
		}

		// var_dump($ieqsr,$drr);
		$count = $ieqs -> where($where) -> count();
		$drr = json_encode($drr);
		$json = '{"code":0,"msg":"","count":'.$count.',"data":'.$drr.'}';
		echo $json;
	}
	...
	public function equipment_subs(){
		...
		foreach($itmr as &$value){
			...
			foreach($data as &$val){
				...
				if($value['deviceid'] == $val['deviceid']){
					...
					$value['device_sub'] = $val['device_sub'] . $val['dispose'];
					$value['device'] = $val['device'];
					$value['supplier'] = $val['supplier'];
					$value['maintenance'] = $val['maintenance'];
		...
		$itmr = json_encode($itmr);
		$json = '{"code":0,"msg":"","count":'.$count.',"data":'.$itmr.'}';
		echo $json;

		// var_dump($itmr);
	}
	public function equipment_network(){
		$equipment['jgh'] = $_GET['jgh'];
		$equipment['equipmentid'] = $_GET['equipmentid'];
		$equipment['stats'] = 0;
		$where['tablemat_classid'] = $_GET['tablemat_classid'];
	...
	public function equipment_networks(){
		$equipment = cookie('equipment');
		// var_dump($equipment);
		$where['tablemat_classid'] = cookie('tablemat_classid');
		$where['stats'] = 0;
		...
		foreach($ieqr as &$value){
			...
			foreach($iswr as &$val){
				...
				if($value['equipment_subid'] == $val['equipment_subid']){

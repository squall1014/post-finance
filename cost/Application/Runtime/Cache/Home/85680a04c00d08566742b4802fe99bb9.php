		    ...
		    <div class="layui-inline">
		      ...
		      <!--卡号待处理；-->
		  		<input type="text" name="pc" list="cardid" id="pcReload" lay-verify="" class="layui-input">
		  			<datalist id="cardid">
         	 		<?php if(is_array($ccorr)): $i = 0; $__LIST__ = $ccorr;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["card"]); ?>"><?php echo ($vo["card"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
         	 	</datalist>
		  		</form>
		    </div>
		    	搜索网点:
		  	<div class="layui-inline">
		  		...
		  		<select name="wd" id="wdReload" lay-verify="" lay-search>
		  			...
		  			<?php if(is_array($ccor)): $i = 0; $__LIST__ = $ccor;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["jgh"]); ?>"><?php echo ($vo["dwname"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
		  		</select>
		  		</form>
		  	</div>
		  		搜索日期：
...
<script>
  ...
  table.render({
    elem: '#test'
    ,url:'<?php echo U("pointcardwdshlist");?>'
    ,toolbar: '#toolbarDemo'
    ,title: '用户数据表'
    ,cols: [[
    ...
    $('.demoTable .layui-btn').on('click', function(){    
  	...
  	table.on('toolbar(test)', function(obj){
	    ...
	    switch(obj.event){
	      ...
	      case 'getCheckstats':
	        ...
	        $.ajax({
	        	type:"post",
	        	url:'<?php echo U("pointcardshbatchstats");?>',
	        	async:true,
	        	data : {
	        		cardoutid : data.data,
	        	},
	  ...
	  form.on('switch(sexDemo)', function(obj){
	    ...
	    $.ajax({
	    	...
	    	url:'<?php echo U("pointcardshstats");?>',

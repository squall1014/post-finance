		      ...
		      <form class="layui-form">
		  		...
		  		<button class="layui-btn" data-type="reload">搜索</button>
					...
					<script type="text/html" id="barDemo">
					  <a class="layui-btn layui-btn-xs layui-btn-danger" lay-event="edit">冲正</a>
...
<script>
  ...
  table.render({
    elem: '#test'
    ,url:'<?php echo U("dwoutboundmodify_ups");?>'
    ,toolbar: '#toolbarDemo'
    ,title: '用户数据表'
    ,cols: [[
      {type: 'checkbox', fixed: 'left'}
      ,{field:'dwoutboundid', title:'ID', width:80, fixed: 'left', unresize: true, sort: true}
      ,{field:'productname', title:'产品名称', width:180, unresize: true, sort: true}
      ,{field:'productdwname', title:'产品所属单位', width:100,sort: true}
      ,{field:'warehouse', title:'产品所在仓库', width:100,sort: true}
	  ,{field:'producttypename', title:'所属类别', width:100, sort: true}
	  ,{field:'date', title:'出库日期', width:120, sort: true}

    //   ,{field:'sjkcquantity', title:'可请领库存', width:100, style :'background-color: #eee; color: red',sort: true}
      ,{field:'outquantity', title:'已出库数量', width:120, sort: true}
	  ...
	  table.on('tool(test)', function(obj){
	    ...
	    if(obj.event === 'del'){
	      layer.confirm('真的删除行么', function(index){
			...
			console.log(data);
			layer.confirm('确认要冲正吗？', function(index){
				$.ajax({
					type:"post",
					
					url:'<?php echo U("dwoutboundmodifysuc_up");?>',
					
					async:true,
					
					data:{
						dwoutboundid: data.dwoutboundid,

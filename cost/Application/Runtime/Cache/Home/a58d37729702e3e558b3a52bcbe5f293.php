...
<script>
  ...
  table.render({
    ...
    ,cols: [[
      ...
      ,{field:'kcapplyquantity', title:'可出库数量', width:120, sort: true}
	    ...
	    if(obj.event === 'del'){
	      ...
	      layer.prompt({
	        ...
	        // ,value: data.sumkcquantity
			  ...
			  if(parseInt(value) > parseInt(data.kcapplyquantity)){
				  alert("出库数量不能大于库存数量")
			  }else{
				$.ajax({
					type:"post",
					
					url:'<?php echo U("dwoutboundsuc_up");?>',
					...
					data:{
						...
						pwid: data.pwid,
						jgh: data.productjgh,
						kcapplyquantity: data.kcapplyquantity,
						ck: value,
					},
				...
				// window.location.reload();
				obj.update({
					kcapplyquantity: data.kcapplyquantity - value,
				})
			  }

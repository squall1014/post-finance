		    ...
		    <div class="layui-inline">
		      <form class="layui-form">
		  		<select name="wh" id="whReload" lay-verify="" lay-search>
		  			<option value="">请选择设备所属</option>
		  			<?php if(is_array($ieqr)): $i = 0; $__LIST__ = $ieqr;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["equipmentid"]); ?>"><?php echo ($vo["equipment"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
		  		</select>
		  		</form>
		    </div>
		    	搜索设备:
		  	<div class="layui-inline">
		  		...
		  		<datalist id="productid">
         	 		<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["productname"]); ?>"><?php echo ($vo["productname"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
         	 	</datalist>
		  		</form>
		  	</div>
		  		搜索日期：
		      ...
		      <form class="layui-form">
		  		...
		  		<button class="layui-btn" data-type="reload">搜索</button>
					...
					<script type="text/html" id="barDemo">
					  <a class="layui-btn layui-btn-xs" lay-event="edit">撤回</a>
...
<script>
  ...
  table.render({
    elem: '#test'
    ,url:'<?php echo U("equipmentmodifys");?>'
    ,toolbar: '#toolbarDemo'
    ,title: '用户数据表'
    ,cols: [[
      {type: 'checkbox', fixed: 'left'}
      ,{field:'device_breakid', title:'ID', width:80, fixed: 'left', unresize: true, sort: true}
      ,{field:'equipment', title:'设备所属', width:120,sort: true}
      ,{field:'device_class', title:'设备大类', width:180, unresize: true, sort: true}
      ,{field:'device_sub', title:'设备名称', width:120,sort: true}
      ,{field:'reason', title:'故障原因', width:120,sort: true}
  	...
  	var $ = layui.$, active = {
       ...
       var dateReload = $('#date');
        table.reload('testReload', {
              where: {
                 equipmentid: whReload.val(),
                 device_class: pcReload.val(),
                 device_sub: dateReload.val()
                 }
            });
          }
       };
	      ...
	      layer.prompt({
	        ...
	        // ,value: data.sumkcquantity
				...
				$.ajax({
					...
					url:'<?php echo U("equipmentmodifysuc");?>',
					
					async:true,
					
					data:{
						device_breakid: data.device_breakid,
						equipment_subid: data.equipment_subid,
						beizhu: value,
					},

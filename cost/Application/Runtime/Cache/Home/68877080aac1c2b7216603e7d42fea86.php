...
<script>
  ...
  table.render({
    elem: '#test'
    ,url:'<?php echo U("applyproductsearch_ups");?>'
    ,toolbar: '#toolbarDemo'
    ,title: '用户数据表'
    ,cols: [[
      {type: 'checkbox', fixed: 'left'}
      ,{field:'applyid', title:'ID', width:80, fixed: 'left', unresize: true, sort: true}
	  ...
	  ,{field:'producttypename', title:'所属类别', width:80, sort: true}
      ,{field:'sjkcquantity', title:'可请领库存', width:100, style :'background-color: #eee; color: red',sort: true}
      ,{field:'applyquantity', title:'已请领数量', width:120, sort: true}
      ,{field:'shenhe', title:'审核状态', width:120, sort: true}
    ...
    $('.demoTable .layui-btn').on('click', function(){    
	  ...
	  table.on('tool(test)', function(obj){
		...
		// 	  if(parseInt(value) > parseInt(data.sjkcquantity)){
		// 		  alert("请领数量不能大于库存数量")
		// 	  }else{
		// 		$.ajax({
		// 			type:"post",
					
		// 			// url:'<?php echo U("applyproductmodifysuc_up");?>',

		    ...
		    <div class="layui-inline">
		      ...
		      <!--卡号待处理；-->
		  		<select name="kh" id="khReload" lay-verify="" lay-search>
		  			...
		  			<?php if(is_array($crr)): $i = 0; $__LIST__ = $crr;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["kh"]); ?>"><?php echo ($vo["kh"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
		  		</select>
...
<script>
  ...
  table.render({
    elem: '#test'
    ,url:'<?php echo U("pointcardwdinlist");?>'
    ,toolbar: '#toolbarDemo'
    ,title: '用户数据表'
    ,cols: [[
    ...
    $('.demoTable .layui-btn').on('click', function(){    
  	...
  	table.on('toolbar(test)', function(obj){
	    ...
	    switch(obj.event){
	      ...
	      case 'getCheckstats':
	        ...
	        $.ajax({
	        	type:"post",
	        	url:'<?php echo U("pointcardinbatchstats");?>',
	        	async:true,
	        	data : {
	        		cardid : data.data,
	        	},
	        	success:function(result){
	        		alert("共有" + result + "条已审核");
	  ...
	  form.on('switch(sexDemo)', function(obj){
	    ...
	    $.ajax({
	    	...
	    	url:'<?php echo U("pointcardinstats");?>',

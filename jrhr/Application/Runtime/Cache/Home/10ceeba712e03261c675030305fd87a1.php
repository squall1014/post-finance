...
<h1><div style="padding: 15px;">余杭区金融积分考核系统</div></h1>
    ...
    <fieldset class="layui-elem-field layui-field-title" style="margin-top: 50px;">
			<legend>每日行为量必报项目</legend>
		</fieldset>
			<div class="layui-card-body">
        <form action="<?php echo U('jractupsuc');?>" enctype="multipart/form-data" method="post" class="layui-form" >
         <table class="layui-table" lay-size="" style="width: 45%">
         	<?php if(is_array($jairrr)): $i = 0; $__LIST__ = $jairrr;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if($vo["actitemid"] == 4): ?><tr>
         		<th style="text-align:center;">支行长是否参与走访</th>
         		<td>
         			<select name="4">
         	...
         	<?php if(is_array($jairr)): $i = 0; $__LIST__ = $jairr;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
         		<th style="text-align:center;"><?php echo ($vo["item"]); ?></th>
         		<td><input type="text" name="<?php echo ($vo["actitemid"]); ?>" id="b<?php echo ($vo["actitemid"]); ?>" onblur="a<?php echo ($vo["actitemid"]); ?>()" placeholder="" autocomplete="off" class="layui-input"> </td>
         		<script type="text/javascript">
         		function a<?php echo ($vo["actitemid"]); ?>(){
         			var total = document.getElementById("b<?php echo ($vo["actitemid"]); ?>").value;
         			var reg = /^[0-9]+\.?[0-9]*$/;
         			...
         			if(!total.match(reg)){
         				if(!total){
         					...
         					alert('请输入正确的数字');
         					document.getElementById("b<?php echo ($vo["actitemid"]); ?>").value = null;
         				}
         	...
         	<tr>
         		<th style="text-align:center;">厅堂活动形式</th>
         		<td>
         			<select name="37">
	         			<option value="">请选择</option>
	              <option value="大制作">大制作类（饺子、粽子、烘焙、寿司等）</option>
	             	<option value="小制作">小制作类（馄饨、毛豆等）</option>
	             	<option value="送早餐">送早餐</option>
            	</select>
         		</td>
         	</tr>
         	<tr>
         		<th style="text-align:center;">生日会活动形式</th>
         		<td>
         			<select name="38">
	         			<option value="">请选择</option>
	              <option value="大制作">大制作类（饺子、粽子、烘焙、寿司等）</option>
         	...
         	<tr>
         		<th style="text-align:center;">村社活动形式</th>
         		<td>
         			<select name="39">
	         			<option value="">请选择</option>
	              <option value="大制作">大制作类（饺子、粽子、烘焙、寿司等）</option>

...
<h1><div style="padding: 15px;">余杭区金融积分考核系统</div></h1>
    ...
    <fieldset class="layui-elem-field layui-field-title" style="margin-top: 50px;">
        ...
        <form action="<?php echo U('jractiveupsuc');?>" enctype="multipart/form-data" method="post" class="layui-form" >
         
         <table class="layui-table" lay-size="" style="width: 35%">
         	...
         	<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
         		...
         		<td><input type="text" required lay-verify="required" name="<?php echo ($vo["activeitemid"]); ?>" id="b<?php echo ($vo["activeitemid"]); ?>" onblur="a<?php echo ($vo["activeitemid"]); ?>()" placeholder="" autocomplete="off" class="layui-input"> </td>
         	</tr>
         	<script type="text/javascript">
         		function a<?php echo ($vo["activeitemid"]); ?>(){
         			var total = document.getElementById("b<?php echo ($vo["activeitemid"]); ?>").value;
         			var reg = /^[0-9]+\.?[0-9]*$/;
         			...
         			if(!total.match(reg)){
         				if(!total){
         					...
         					alert('请输入正确的数字');
         					document.getElementById("b<?php echo ($vo["activeitemid"]); ?>").value = null;
         				}

...
//	    	}
    ...
    public function jrpointperspays(){
    	...
    	//$datas = $User -> query("select hr_jrpointsum.jgh,hr_jrpointsum.zhiwud,hr_jrpointsum.persname from hr_jrpointsum where hr_jrpointsum.shenhe = 1 and date >= '$date' and date <= '$dates' order by hr_jrpointsum.jgh desc limit 10");
    	$datass = $User -> query("select persname,gonghao,substring_index(group_concat(jgh order by jgh asc),',',1) as jgh from hr_jrpointsum where zhiwud <> 75 and date >= '$date' and date <= '$dates' and gonghao <> '$qjr' and gonghao not in ($qjrrr) and shenhe = 1 group by persname,gonghao");
    	//$datass = $User -> query("select persname,substring_index(group_concat(jgh order by jgh asc),',',1) as jgh from hr_jrpointsum group by persname");
